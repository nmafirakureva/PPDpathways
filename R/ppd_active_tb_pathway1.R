...
if(file.exists(fn)){
  ...
  LabelFromData(SOC_ATB,labz[,..labdat]) #add label data
  ## NOTE checks need redoing
  SOC_ATB$Set(check=1)
  SOC_ATB$Set(check=0,filterFun=function(x) length(x$children)>0)
  ## save out
  tree2file(SOC_ATB,filename = here('indata/CSV/SOC_ATB2.csv'),
