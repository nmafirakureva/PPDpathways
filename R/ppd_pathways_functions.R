...
MLH <- function(dat) {
  ...
  setnames(L, nnmz, lnmz)
  setnames(M, nnmz, mnmz)
  setnames(H, nnmz, hnmz)
  list(L = L, M = M, H = H)
}
...
outsummary <- function(out) {
  ...
  ## pretty version
  pouts <- outs[, .(
    costperATT.soc = brkt(costperATT.soc.mid, costperATT.soc.lo, costperATT.soc.hi),
